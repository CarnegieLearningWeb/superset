...
class SupersetPages(BaseSupersetView):
    """User-facing SPA page routes, served directly under the app root.

    route_base="" (rather than the FAB default of "/superset" derived from the class name) so
    that with SUPERSET_APP_ROOT="/superset" these resolve to a single prefix
    (/superset/welcome/, /superset/dashboard/<id>/) instead of the doubled /superset/superset/.
    class_permission_name="Superset" keeps the existing can_* permissions intact -- these
    methods were previously defined on the Superset view, so the registered permissions
    (can_dashboard / can_welcome / ...) are unchanged.
    """
...
class Superset(BaseSupersetView):
    ...
    Holds the legacy, mostly-deprecated JSON/redirect endpoints. FAB derives this class's
    route_base from the class name ("/superset"), so with SUPERSET_APP_ROOT="/superset" these
    stay at their historical external path (/superset/superset/...) and frontend JS keeps
    working. The user-facing SPA *page* routes (welcome, dashboard, ...) live in SupersetPages
    below with route_base="" so they resolve to a single /superset/ prefix.
    """
